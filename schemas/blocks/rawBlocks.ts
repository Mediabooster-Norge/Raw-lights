...
export const rawStoryHero = defineType({
  ...
  fields: [defineField({ name: 'eyebrow', title: 'Eyebrow', type: 'string' }), defineField({ name: 'headingLines', title: 'Heading lines', type: 'array', of: [{ type: 'string' }], validation: (Rule) => Rule.required().min(1).max(2) }), defineField({ name: 'layout', title: 'Hero layout', type: 'string', options: { list: [{ title: 'Story', value: 'story' }, { title: 'About', value: 'about' }] }, initialValue: 'story' }), defineField({ name: 'chapter', title: 'Chapter label', type: 'string', hidden: ({ parent }) => parent?.layout === 'about' }), defineField({ name: 'chapterTitle', title: 'Chapter title', type: 'string', hidden: ({ parent }) => parent?.layout === 'about' }), defineField({ name: 'aside', title: 'Aside text', type: 'text', rows: 2, hidden: ({ parent }) => parent?.layout === 'about' }), defineField({ name: 'showScrollCue', title: 'Vis scroll-indikator', type: 'boolean', initialValue: true, description: 'Viser teksten og den animerte linjen under hero-overskriften.' }), defineField({ name: 'showChapters', title: 'Show story chapter navigation', type: 'boolean', initialValue: false, hidden: ({ parent }) => parent?.layout === 'about' }), image('image', 'Background image'), animation],
