...
describe('revalidateSpec', () => {
  ...
  it('revalidates the product catalog and localized product route', () => {
    ...
    assert.ok(spec.paths.some((item) => item.path === '/produkter'))
    assert.ok(spec.paths.some((item) => item.path === '/en/products'))
    assert.ok(spec.paths.some((item) => item.path === '/produkter/raw-carbon-9'))
