...
export function ProductPage({ product }: ProductPageProps) {
  ...
  return <>
    ...
    {product.relatedProducts?.length > 0 && <section className="raw-related"><div className="raw-section"><p className="raw-eyebrow">{copy.productRelatedEyebrow}</p><h2 className="raw-display">{copy.productRelatedHeading}</h2></div><div className="raw-catalog">{product.relatedProducts.map((related: any) => <LocaleLink key={related._id} className="raw-card" href={productPath(locale, related.slug)}>{related.heroImage && <SanityImage image={related.heroImage} width={760} height={560} sizes="(min-width: 900px) 33vw, 100vw" />}<p className="raw-kicker">{related.sku}</p><h3>{related.title}</h3><p>{related.excerpt}</p></LocaleLink>)}</div></section>}
  </>
}
