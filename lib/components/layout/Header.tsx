...
import type { LanguageUrls } from '@/lib/i18n/languageSwitcher'

type NavItem = { label: string; link?: any; children?: NavItem[] }
type HeaderProps = { logo?: any; logoLight?: any; enableLightMode?: boolean; mainNav?: NavItem[]; headerCta?: { link: any }; homeHref?: string; languageUrls?: LanguageUrls }
...
export function Header({ logo, logoLight, enableLightMode = false, mainNav, headerCta, homeHref = '/', languageUrls }: HeaderProps) {
  ...
  const navigation = (onClick?: () => void) => mainNav?.map((item) => (
    <div key={item.label}>
      <SanityLink link={item.link} onClick={onClick} prefetch>{item.label}</SanityLink>
      {item.children?.map((child) => <SanityLink key={child.label} link={child.link} onClick={onClick} prefetch>{child.label}</SanityLink>)}
    </div>
  ))

  return <>
    <header className={`raw-header ${open ? 'is-menu-open' : ''}`}>
      <LocaleLink href={homeHref} className="raw-header__logo" aria-label="RAW Lights home" prefetch>{logo ? <Logo logo={logo} logoLight={logoLight} className="h-10 w-auto" /> : <span>RAW</span>}</LocaleLink>
      <nav className="raw-header__nav" aria-label="Main navigation">{navigation()}</nav>
      <div className="raw-header__end"><LanguageSwitcher initialUrls={languageUrls} />{enableLightMode && <ThemeToggle />}{headerCta?.link && <SanityLink link={headerCta.link} className="raw-button raw-button--small" prefetch />}<button type="button" className="raw-menu-button" onClick={() => setOpen(!open)} aria-label={open ? t(locale, 'closeMenu') : t(locale, 'menu')} aria-expanded={open}><span /></button></div>
    </header>
    <nav className={`raw-mobile-menu ${open ? 'is-open' : ''}`} aria-label="Mobile navigation" aria-hidden={!open}>
      ...
      {headerCta?.link && <SanityLink link={headerCta.link} onClick={() => setOpen(false)} className="raw-button raw-button--primary raw-mobile-menu__cta" prefetch />}
    </nav>
  </>
}
